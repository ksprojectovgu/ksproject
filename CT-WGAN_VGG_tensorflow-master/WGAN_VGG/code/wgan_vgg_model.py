...
class wganVgg(object):
    def __init__(self, sess, args):
        ...
        print("Train Patient Number",self.train_patient_no)
        ...
        if args.phase == 'train':
            ...
            self.image_loader(args.LDCT_path)
            ...
            print(self.z_i, self.x_i)
        else:
            self.test_image_loader(self.test_patient_no)
            print('data load complete !!!, {}, N_test : {}'.format(time.time() - t1, len(self.test_image_loader.LDCT_image_name)))
            self.z_i = tf.compat.v1.placeholder(tf.float32, [None, args.patch_size, args.patch_size, args.img_channel], name = 'whole_LDCT')
            self.x_i = tf.compat.v1.placeholder(tf.float32, [None, args.patch_size, args.patch_size, args.img_channel], name = 'whole_LDCT')

        """
        build model
        """
        #### image placehold  (patch image, whole image)
        self.whole_z = tf.compat.v1.placeholder(tf.float32, [1, args.whole_size, args.whole_size, args.img_channel], name = 'whole_LDCT')
        self.whole_x = tf.compat.v1.placeholder(tf.float32, [1, args.whole_size, args.whole_size, args.img_channel], name = 'whole_NDCT')
        ...
        #### loss define
        #gradients penalty
        self.epsilon = tf.random.uniform([], 0.0, 1.0)
        ...
        print("1_PerceptualLoss_VGG", self.vgg_perc_loss)
        print("2_DiscriminatorLoss_WGAN", self.D_loss)
        print("3_D_loss_disc", d_loss)
        print("4_D_loss_gradient_penalty", grad_penal)
        print("GeneratorLoss", self.G_loss)
        # self.summary_all_loss = tf.summary.merge([self.summary_vgg_perc_loss, self.summary_d_loss_all, self.summary_d_loss_1, self.summary_d_loss_2, self.summary_g_loss])
        ...
        with tf.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
            self.d_adam = tf.optimizers.Adam(learning_rate= args.alpha).minimize(self.D_loss, var_list = self.d_vars,tape=tf.GradientTape())
            self.g_adam = tf.optimizers.Adam(learning_rate= args.alpha).minimize(self.G_loss, var_list = self.g_vars,tape=tf.GradientTape())
