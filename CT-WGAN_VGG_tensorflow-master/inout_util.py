...
class DCMDataLoader(object):
    ...
    def __call__(self, patent_no_list):
       ...
       #      os.path.join(self.dcm_path, 'N005', self.LDCT_image_path+'/'), \
       # os.path.join(self.dcm_path, 'N005', self.NDCT_image_path+'/')

        #glob(os.path.join(self.dcm_path, 'N005', self.LDCT_image_path+'/')), \
        #glob(os.path.join(self.dcm_path, 'N005', self.NDCT_image_path+'/'))
        #print(glob(os.path.join(self.dcm_path, 'N005', self.LDCT_image_path, '1-01.'+self.extension)))
        #load images
        org_LDCT_images  = self.get_pixels_hu(self.load_scan(self.LDCT_image_path))
       # print(org_LDCT_images)
        org_NDCT_images = self.get_pixels_hu(self.load_scan(self.LDCT_image_path))
      #  print(org_LDCT_images)

        #CT slice name
        self.LDCT_image_name.extend("N900c")
        self.NDCT_image_name.extend("N900c")
    ...
    def load_scan(self, path):
        ...
        slices, nrrd_header = nrrd.read(path)
        print(slices.shape)

        return slices

        #slices.sort(lambda x: float(x.ImagePositionPatient[2]))
        #print(slices)
        # try:
        #     slice_thickness =\
    ...
    def get_pixels_hu(self, slices):
        ...
        #     intercept = slices[slice_number]
        #     slope = slices[slice_number]
        #     print(slope)
        #     if slope != 1:
    ...
    def input_pipeline(self, sess, image_size, end_point, depth = 1):
        ...
        queue = tf.queue.FIFOQueue(capacity=self.capacity, dtypes=[tf.float32, tf.float32], \
            ...
            if  self.model == 'cyclegan':#only cyclegan (cycelgain-identity:random patch))
                ...
                while not coord.should_stop():
                    ...
                    while start_pos < len(self.LDCT_index):
                        ...
                        sess.run(enqueue_op, feed_dict={queue_input: np.expand_dims(raw_LDCT_chunk, axis=-1),queue_output: np.expand_dims(raw_NDCT_chunk, axis=-1)})
                ...
                while not coord.should_stop():
                    ...
                    for i in range(enqueue_size):
                        ...
                        LDCT_imgs.extend(pat_LDCT.tolist())
                        NDCT_imgs.extend(pat_NDCT.tolist())
                        #print(type(LDCT_imgs))
                        # print(NDCT_imgs)
                    ...
                    sess.run(enqueue_op, feed_dict={queue_input: np.array(LDCT_imgs),queue_output: np.array(NDCT_imgs)})
                    self.step += 1
                if self.step > end_point:
                    coord.request_stop()
                sess.run(close_op)
